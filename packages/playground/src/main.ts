...
function runConversion() {
  ...
  try {
    ...
    lastResult = { error: e instanceof Error ? `${e.name}: ${e.message}` : String(e) };
    setStatus('error');
  }
  renderOutput();
}
...
function renderOutput() {
  const mdastTab = document.querySelector<HTMLButtonElement>('[data-tab="mdast"]')!;
  mdastTab.disabled = activeFormat !== 'markdown';
  if (activeFormat !== 'markdown' && activeTab === 'mdast') setTab('xml', false);
  ...
  switch (activeTab) {
    case 'xml':
      outputEl.textContent = lastResult.xml ? indentXml(lastResult.xml) : '(no output)';
      break;
    case 'ptxast':
...
function stripForDisplay(node: unknown): unknown {
  ...
  if (node !== null && typeof node === 'object') {
    ...
    return Object.fromEntries(Object.entries(rest).map(([k, v]) => [k, stripForDisplay(v)]));
  }
  return node;
}
...
formatSelect.addEventListener('change', () => setFormat(formatSelect.value as Format));
