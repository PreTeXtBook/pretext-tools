...
describe("detectConvertibleFormat", () => {
  // The snippet detector itself is specified in @pretextbook/import; these are
  // the cases that actually reach the paste provider — fragments copied from
  // the middle of a document, carrying no \documentclass or \section.
  it.each([
    ["an environment", "\\begin{theorem}\nHi\n\\end{theorem}", "latex"],
    ["inline math", "Find the derivative of $x^2$ here.", "latex"],
    ["a command", "Let $G$ be a \\emph{group}.", "latex"],
    ["a heading", "# Title\n\nSome text.", "markdown"],
    ["a bullet list", "- One\n- Two", "markdown"],
    ["bold", "This is **important**.", "markdown"],
  ])("detects %s", (_label, text, expected) => {
    expect(detectConvertibleFormat(text)).toBe(expected);
  });

  it.each([
    ["plain prose", "Just a sentence I copied."],
    ["blank", "   "],
    ["empty", ""],
    ["PreTeXt markup", "<p>Already <em>PreTeXt</em>.</p>"],
    ["currency", "The book costs $5 and the workbook costs $7."],
  ])("leaves %s alone", (_label, text) => {
    expect(detectConvertibleFormat(text)).toBeUndefined();
  });
});
