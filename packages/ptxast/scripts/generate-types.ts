/**
 * Generate xast-style TypeScript interfaces for PreTeXt elements from the
 * official RELAX-NG schema. Run via:
 *
 *   npx tsx packages/ptxast/scripts/generate-types.ts
 *
 * Writes output to packages/ptxast/src/types/generated-interfaces.ts
 */

import fs from "node:fs/promises";
import path from "node:path";
import { fileURLToPath } from "node:url";
import { unified } from "unified";
import { fromXml } from "xast-util-from-xml";
import { toXml } from "xast-util-to-xml";
import type { Root } from "xast";

// Import from the relax-ng-to-typescript TypeScript source (no built dist).
import {
  removePositionPlugin,
  doSimplificationPlugin,
  makeTypesForGrammar,
  renameRefsPlugin,
} from "../../../node_modules/relax-ng-to-typescript/src/index.ts";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const workspaceRoot = path.resolve(__dirname, "../../..");

const canonicalRngPath = path.join(
  workspaceRoot,
  "packages",
  "vscode-extension",
  "assets",
  "schema",
  "pretext-dev.rng",
);

// Backward-compatibility with older repository layouts.
const legacyRngPath = path.join(
  workspaceRoot,
  "extension",
  "assets",
  "schema",
  "pretext-dev.rng",
);

const outPath = path.join(__dirname, "../src/types/generated-interfaces.ts");

async function getExistingRngPath(): Promise<string> {
  try {
    await fs.access(canonicalRngPath);
    return canonicalRngPath;
  } catch {
    // continue to legacy location
  }

  try {
    await fs.access(legacyRngPath);
    return legacyRngPath;
  } catch {
    throw new Error(
      `Could not find pretext-dev.rng in either expected location:\n- ${canonicalRngPath}\n- ${legacyRngPath}`,
    );
  }
}

function unifiedXml() {
  return unified()
    .use(function () {
      // @ts-ignore
      this.Parser = fromXml;
    })
    .use(function () {
      // @ts-ignore
      this.Compiler = toXml;
    });
}

async function resolveIncludes(rngFilePath: string): Promise<string> {
  let content = await fs.readFile(rngFilePath, "utf-8");
  const includeRegex = /<include\s+href=("|')(.*?)\1\s*\/>/g;
  let match: RegExpExecArray | null;
  while ((match = includeRegex.exec(content)) !== null) {
    const includePath = path.join(path.dirname(rngFilePath), match[2]);
    let included = await fs.readFile(includePath, "utf-8");
    included = included.replace(/<\?xml.*?\?>/, "");
    included = included.replace(/<grammar[^>]*>/, "");
    included = included.replace(/<\/grammar>/, "");
    content = content.replace(match[0], included);
  }
  return content;
}

async function main() {
  const rngPath = await getExistingRngPath();
  console.log(`Reading RNG schema from: ${rngPath}`);
  const source = await resolveIncludes(rngPath);

  const processor = unifiedXml()
    .use(removePositionPlugin)
    .use(doSimplificationPlugin);

  const parsed = processor.parse(source);
  let ast = processor.runSync(parsed as any as Root);
  ast = unifiedXml().use(renameRefsPlugin).runSync(ast);

  const grammarTypes = makeTypesForGrammar((ast as Root).children[0]);

  const header = `// Generated by packages/ptxast/scripts/generate-types.ts from packages/vscode-extension/assets/schema/pretext-dev.rng.
// Do not edit manually. Re-generate with: npx tsx packages/ptxast/scripts/generate-types.ts
// (or run: npm run refresh:schemas)
`;

  // Post-process 1: Replace `extends Omit<XMLElement, "attributes"> {` with
  // `extends XMLElement {` and inject an empty attributes declaration. This
  // ensures all generated element interfaces are proper subtypes of Element.
  // Step 2 below will add the required index signature to this block.
  const step1 = grammarTypes.typescriptStr.replace(
    /extends Omit<XMLElement, "attributes"> \{\n([ \t]+)(name: [^\n]+;\n)/g,
    (_, indent, nameDecl) =>
      `extends XMLElement {\n${indent}${nameDecl}${indent}attributes: {};\n`,
  );

  // Post-process 2: inject the required index signature into every attributes
  // object so generated interfaces are compatible with @types/xast's Attributes
  // type (which requires `{ [key: string]: string | null | undefined }`).
  const processedTypes = step1.replace(
    /^([ \t]+)(attributes\??:\s*\{)/gm,
    (_, indent, rest) =>
      `${indent}${rest}\n${indent}  [key: string]: string | null | undefined;`,
  );

  const output = header + processedTypes;
  await fs.writeFile(outPath, output, "utf-8");
  console.log(`Wrote xast interfaces to: ${outPath}`);
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
